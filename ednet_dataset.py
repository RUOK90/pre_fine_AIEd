...
class torch_EdNetDataSet(data.Dataset):
    def __init__(self, q_info_dic, user_inter_path, user_windows):
        self._q_info_dic = q_info_dic
        self._user_inter_path = user_inter_path
...
def torch_get_user_data(user_inter_path):
    with open(user_inter_path, "rb") as f_r:
        uid2inters = pkl.load(f_r)
...
def torch_preprocess_inters(inters, window_idx):
    (
        qids,
        parts,
        is_corrects,
...
def torch_get_masked_features(features):
    masked_features = {}
    seq_size = len(list(features.values())[0])
...
def torch_get_padded_features(features, return_padding_mask):
    seq_size = len(list(features.values())[0])
    num_pads = max(ARGS.max_seq_size - seq_size, 0)
    for name, feature in features.items():
...
def torch_get_padded_masks(masks):
    seq_size = len(masks)
    num_pads = max(ARGS.max_seq_size - seq_size, 0)
