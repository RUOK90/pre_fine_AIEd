...
class MultiHeadedAttention(torch.nn.Module):
    ...
    def __init__(self, d_h, d_model, dropout=0.1):
        ...
        self.d_h = d_h
        self.linears = am_util.clones(torch.nn.Linear(d_model, d_model), 4)
        self.attn = None
...
class Encoder(torch.nn.Module):
    ...
    def __init__(self, layer, N):
        super().__init__()
        self.layers = am_util.clones(layer, N)
...
class EncoderEmbedding(torch.nn.Module):
    ...
    def __init__(self, embedding_by_feature, seq_size, dropout):
        super().__init__()
...
class ScoreGenerator(torch.nn.Module):
    ...
    def _fully_connected(self, am_outputs):
        ...
        return outputs.squeeze(-1), torch.sigmoid(outputs).squeeze(-1)
